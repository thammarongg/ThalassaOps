...
5. Adding a comment does not carry `expected_version` and does not advance the
   ...
   make every comment invalidate every other responder's read exactly when an
   incident is busiest.
...
7. The four association tabs render only what is already associated with the
   ...
   associate more signals after creation.
8. The Incident Card is an in-application view plus a bounded clipboard copy. No
   file export, no link, no image.
...
- an in-application Incident Card with a bounded clipboard copy;
...
| `IncidentCard.tsx` | Bounded summary and clipboard copy. |
...
Copying the Incident Card places incident content outside the application, where
...
1. **Comments require full incident management rights.** Every incident write
   ...
   therefore either do everything or nothing. Splitting a comment permission is
   deferred to Sprint 20 (Policy Center), which owns the permission model.
2. **Associations are frozen.** A responder who realises another alert belongs
   to the incident cannot attach it. The workspace is exactly where that need
   arises, so this will be felt.
3. **Failed evidence resolution cannot name the identifier.** Section 11. The
   fix belongs with the Sprint 18 evidence and redaction work.
4. **No Evidence Tide Line.** Section 3.2.
